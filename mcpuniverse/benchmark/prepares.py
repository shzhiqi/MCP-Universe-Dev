...
async def set_postgres_database(context: Context = None, database: str = None, category: str = None, **kwargs):
    """
    Dynamically set POSTGRES_DATABASE and POSTGRES_ADDRESS for a specific database.
    
    This function will automatically use the task's category as the database name if database is not specified.
    ...
    if postgres_address:
        ...
        if len(parts) == 2:
            ...
            logger.info(f"Set POSTGRES_DATABASE to '{db_name}' and updated POSTGRES_ADDRESS")
...
async def prepare_vector_database(context: Context = None, **kwargs):
    """
    ...
    try:
        ...
        if not script_path.exists():
            raise FileNotFoundError(f"Prepare environment script not found: {script_path}")
        ...
        logger.error(f"Failed to prepare vector database: {e}")
        raise


@prepare_func("prepare_rls_business_access")
async def prepare_rls_business_access(context: Context = None, **kwargs):
    """
    ...
    try:
        ...
        logger.error(f"Failed to prepare RLS business access: {e}")
        raise


@prepare_func("prepare_user_permission_audit")
async def prepare_user_permission_audit(context: Context = None, **kwargs):
    """
    ...
    try:
        ...
        logger.error(f"Failed to prepare user permission audit: {e}")
        raise


@prepare_func("download_filesystem_environment")
async def download_filesystem_environment(context: Context = None, category: str = None, **kwargs):
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                raise RuntimeError("unzip command not found. Please install unzip.")
            ...
            logger.info(f"| ✓ Successfully downloaded: {target_dir}")
        ...
        logger.error(f"Failed to download filesystem environment: {e}")
        raise
...
async def mcpmark_github_setup(context: Context = None, category: str = None, **kwargs):
    """
    ...
    try:
        ...
        logger.info(f"Setting up GitHub environment for task: {mock_task.name}")
        ...
        if success:
            ...
            logger.info(f"GitHub environment setup completed for task: {mock_task.name}")
            return f"GitHub environment setup completed for task: {mock_task.name}"
        else:
            raise RuntimeError("GitHub setup failed")
            
    except Exception as e:
        logger.error(f"Failed to setup GitHub environment: {e}")
        raise


@prepare_func("mcpmark_notion_setup")
async def mcpmark_notion_setup(context: Context = None, category: str = None, **kwargs):
    """
    ...
    try:
        ...
        logger.info(f"Setting up Notion environment for task: {mock_task.name}")
        ...
        if success:
            ...
            if context:
                ...
                if hasattr(mock_task, 'duplicated_initial_state_url') and mock_task.duplicated_initial_state_url:
                    context.env["MCPMARK_NOTION_PAGE_URL"] = mock_task.duplicated_initial_state_url
            
            logger.info(f"Notion environment setup completed for task: {mock_task.name}")
            return f"Notion environment setup completed for task: {mock_task.name}"
        else:
            raise RuntimeError("Notion setup failed")
            
    except Exception as e:
        logger.error(f"Failed to setup Notion environment: {e}")
        raise


@prepare_func("mcpmark_filesystem_setup")
async def mcpmark_filesystem_setup(context: Context = None, category: str = None, **kwargs):
    """
    ...
    try:
        ...
        logger.info(f"Setting up Filesystem environment for task: {mock_task.name}")
        ...
        if success:
            ...
            logger.info(f"Filesystem environment setup completed for task: {mock_task.name}")
            return f"Filesystem environment setup completed for task: {mock_task.name}"
        else:
            raise RuntimeError("Filesystem setup failed")
            
    except Exception as e:
        logger.error(f"Failed to setup Filesystem environment: {e}")
        raise


@prepare_func("mcpmark_playwright_setup")
async def mcpmark_playwright_setup(context: Context = None, category: str = None, **kwargs):
    """
    ...
    try:
        ...
        logger.info(f"Setting up Playwright environment for task: {mock_task.name}")
        
        # Initialize state manager
        state_manager = PlaywrightStateManager(
            browser=context.get_env("PLAYWRIGHT_BROWSER", "chromium") if context else "chromium",
            headless=context.get_env("PLAYWRIGHT_HEADLESS", "true").lower() == "true" if context else True
        )
        ...
        if success:
            ...
            logger.info(f"Playwright environment setup completed for task: {mock_task.name}")
            return f"Playwright environment setup completed for task: {mock_task.name}"
        else:
            raise RuntimeError("Playwright setup failed")
            
    except Exception as e:
        logger.error(f"Failed to setup Playwright environment: {e}")
        raise


@prepare_func("mcpmark_playwright_webarena_setup")
async def mcpmark_playwright_webarena_setup(context: Context = None, category: str = None, **kwargs):
    """
    ...
    try:
        ...
        # Import mcpmark modules
        from src.mcp_services.playwright_webarena.playwright_state_manager import PlaywrightStateManager
        ...
        logger.info(f"Setting up Playwright WebArena environment for task: {mock_task.name}")
        ...
        if success:
            ...
            logger.info(f"Playwright WebArena environment setup completed for task: {mock_task.name}")
            return f"Playwright WebArena environment setup completed for task: {mock_task.name}"
        else:
            raise RuntimeError("Playwright WebArena setup failed")
            
    except Exception as e:
        logger.error(f"Failed to setup Playwright WebArena environment: {e}")
        raise


@prepare_func("mcpmark_postgres_setup")
async def mcpmark_postgres_setup(context: Context = None, category: str = None, **kwargs):
    """
    ...
    try:
        ...
        logger.info(f"Setting up Postgres environment for task: {mock_task.name}")
        ...
        if success:
            ...
            if context:
                ...
                if hasattr(mock_task, 'database_name') and mock_task.database_name:
                    ...
                    logger.info(f"Set POSTGRES_DATABASE to: {mock_task.database_name}")
            ...
            logger.info(f"Postgres environment setup completed for task: {mock_task.name}")
            logger.info(f"Database created: {mock_task.database_name}")
            return f"Postgres environment setup completed. Database: {mock_task.database_name}"
        else:
            raise RuntimeError("Postgres setup failed")
            
    except Exception as e:
        logger.error(f"Failed to setup Postgres environment: {e}")
        raise
