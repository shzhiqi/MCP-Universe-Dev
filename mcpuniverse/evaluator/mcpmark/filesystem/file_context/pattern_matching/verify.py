...
def verify_matches_are_correct(test_dir: Path) -> tuple:
    ...
    try:
        ...
        if not content:
            ...
            if expected_matches:
                ...
                return False, f"Answer file is empty but matches should exist: {list(expected_matches.keys())}"
            else:
        ...
        for filename, start_pos in answer_matches.items():
            if filename not in expected_matches:
                ...
                return False, f"File {filename} listed in answer but has no valid 30+ character match"
            ...
            if start_pos != expected_start:
                ...
                return False, f"Incorrect match position for {filename}. Expected: {expected_start}, Found: {start_pos}"
...
def verify_match_length_is_30_plus(test_dir: Path) -> tuple:
    ...
    try:
        ...
        for line in lines:
            ...
            if len(longest_match) < 30:
                ...
                return False, f"Match in {filename} is {len(longest_match)} characters, less than 30"
