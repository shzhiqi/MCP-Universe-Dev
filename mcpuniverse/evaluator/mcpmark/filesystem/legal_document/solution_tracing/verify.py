    ...
    try:
        with open(output_file, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            if len(header) != 5:  # First column (can be anything) + 4 clauses
                ...
                return False, f"Header row has incorrect number of columns: {len(header)}, expected 5"
            ...
            for i, row in enumerate(rows[1:], 1):
                if len(row) != 5:
                    ...
                    return False, f"Data row {i} has incorrect number of columns: {len(row)}, expected 5"
                        ...
                        for j, value in enumerate(values, 1):
                            try:
                                ...
                                if int_val < 5 or int_val > 8:
                                    ...
                                    return False, f"Row {i}, column {j}: version number '{value}' is out of expected range [5-8]"
                            except ValueError:
                                print(f"❌ Row {i}, column {j}: non-integer version number '{value}'")
                                return False, f"Row {i}, column {j}: non-integer version number '{value}'"
