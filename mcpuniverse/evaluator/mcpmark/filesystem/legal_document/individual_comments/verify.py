...
def verify_output_file_exists(test_dir: Path) -> tuple:
    ...
    if not output_file.exists():
        ...
        return False, "File 'individual_comment.csv' not found"
    ...
    try:
        with open(output_file, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            if len(header) != 7:  # First column (can be anything) + 6 clauses
                ...
                return False, f"Header row has incorrect number of columns: {len(header)}, expected 7"
            ...
            for i, row in enumerate(rows[1:], 1):
                if len(row) != 7:
                    ...
                    return False, f"Data row {i} has incorrect number of columns: {len(row)}, expected 7"
...
def verify_csv_content(test_dir: Path) -> tuple:
    ...
    try:
        with open(output_file, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            if missing_names:
                ...
                return False, f"Missing expected names: {missing_names}"
            ...
            if extra_names:
                ...
                return False, f"Unexpected extra names: {extra_names}"
            ...
            for name, expected_values in expected_data.items():
                ...
                if actual_values != expected_values:
                    ...
                    return False, f"Values mismatch for {name}"
...
def verify_data_accuracy(test_dir: Path) -> tuple:
    ...
    try:
                    ...
                    for j, value in enumerate(values, 1):
                        try:
                            ...
                            if int_val < 0:
                                ...
                                return False, f"Row {i}, column {j}: negative value '{value}' for {name}"
                        except ValueError:
                            print(f"❌ Row {i}, column {j}: non-integer value '{value}' for {name}")
                            return False, f"Row {i}, column {j}: non-integer value '{value}' for {name}"
        ...
        return False, f"Error verifying data accuracy: {e}"
