...
def verify_song_ranking_format(test_dir: Path) -> tuple:
    ...
    try:
        ...
        for i in range(1, len(scores)):
            if scores[i] > scores[i-1]:
                ...
                return False, f"Scores not in descending order: {scores[i-1]} < {scores[i]} at line {i+1}"
...
def verify_top_5_songs(test_dir: Path) -> tuple:
    ...
    try:
        ...
        if not order_valid:
            ...
            return False, f"Top 5 songs order is invalid. Found: {found_top_5}, Expected one of: {valid_orders}"
