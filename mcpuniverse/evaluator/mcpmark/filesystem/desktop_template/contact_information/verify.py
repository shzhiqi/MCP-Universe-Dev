...
def verify_csv_structure(test_dir: Path) -> tuple:
...
def verify_csv_content_accuracy(test_dir: Path) -> tuple:
                ...
                for key, expected_value in expected.items():
                    if key in row:
                        ...
                        if actual_value != expected_value:
                            ...
                            return False, f"Entry '{row_name}', column '{key}': expected '{expected_value}', got '{actual_value}'"
                    else:
...
def verify_file_locations(test_dir: Path) -> tuple:
    ...
    if contact_file.parent != test_dir:
        print(f"❌ contact_info.csv is not in main directory: {contact_file}")
        return False, f"contact_info.csv is not in main directory: {contact_file}"
    
    if answer_file.parent != test_dir:
        print(f"❌ answer.txt is not in main directory: {answer_file}")
        return False, f"answer.txt is not in main directory: {answer_file}"
