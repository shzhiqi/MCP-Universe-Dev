    ...
    for expected_month, days in expected_structure.items():
        ...
        for day, expected_files in days.items():
            ...
            if day_dir is None:
                ...
                return False, f"Day directory '{month_dir.name}/{day}' not found. Expected one of: {valid_day_names}"
    ...
    for expected_month, days in expected_structure.items():
        ...
        for day, expected_files in days.items():
            ...
            if unexpected_files:
                ...
                return False, f"Unexpected files in '{month_dir.name}/{day_dir.name}': {unexpected_files}"
    
    print("✅ All files are in correct directories")
    return True, ""

def verify_metadata_analysis_files(test_dir: Path) -> tuple:
        ...
        for day, expected_files in days.items():
            ...
            try:
                ...
                if len(lines) != 2:
                    ...
                    return False, f"metadata_analyse.txt in '{month_dir.name}/{day_dir.name}' has {len(lines)} lines, expected 2"
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if expected_month == "07" and day == "09":
                        ...
                        if line_num == 1:  # First line should be bear.jpg
                            ...
                            if not any(filename in line_lower for filename in expected_filenames):
                                ...
                                return False, f"Line {line_num} in '{month_dir.name}/{day_dir.name}' should contain one of {expected_filenames}"
                    ...
                    if expected_filename and expected_filename not in line_lower:
                        ...
                        return False, f"Line {line_num} in '{month_dir.name}/{day_dir.name}' should contain '{expected_filename}'"
                    ...
                    if month_letters and not any(letter in line_lower for letter in month_letters):
                        ...
                        return False, f"Line {line_num} in '{month_dir.name}/{day_dir.name}' should contain month letters"
                    
                    # Check year (2025)
                    if "2025" not in line_lower:
                        print(f"❌ Line {line_num} in '{month_dir.name}/{day_dir.name}' should contain '2025': {line}")
                        return False, f"Line {line_num} in '{month_dir.name}/{day_dir.name}' should contain '2025'"
                    ...
                    if not any(day_name in line_lower for day_name in valid_day_names):
                        ...
                        return False, f"Line {line_num} in '{month_dir.name}/{day_dir.name}' should contain day '{day}'"
                
            except Exception as e:
                print(f"❌ Error reading metadata_analyse.txt in '{month_dir.name}/{day_dir.name}': {e}")
                return False, f"Error reading metadata_analyse.txt in '{month_dir.name}/{day_dir.name}': {e}"
