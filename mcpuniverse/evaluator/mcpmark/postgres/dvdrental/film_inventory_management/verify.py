...
def check_available_films_table(conn) -> tuple[bool, str]:
    ...
    with conn.cursor() as cur:
        ...
        if len(actual_results) != len(expected_results):
            ...
            return False, f"available_films table has {len(actual_results)} records, expected {len(expected_results)}"
...
def check_inventory_cleanup(conn) -> tuple[bool, str]:
    ...
    with conn.cursor() as cur:
        ...
        if remaining_count > 0:
            ...
            return False, f"Found {remaining_count} inventory records that should have been deleted"
...
def check_summary_table(conn) -> tuple[bool, str]:
    ...
    with conn.cursor() as cur:
        ...
        if len(actual_results) != len(expected_results):
            ...
            return False, f"film_inventory_summary table has {len(actual_results)} records, expected {len(expected_results)}"
